    ...
    pageData.forEach((item, index) => {
        ...
        tr.innerHTML = `
            ...
            <td class="actions-cell actions-cell-wrapper" style="text-align: center; white-space: nowrap;">
                <button class="action-btn" onclick="toggleItemTallyStatus('${item.id}')" title="${isTallied ? 'Tallied with Payment Voucher by ' + (item.talliedBy || 'Accounting') + ' (Click to mark Pending)' : 'Pending Voucher Tally (Click to verify & tally)'}" style="color: ${isTallied ? '#4ade80' : '#fbbf24'}; margin-right: 4px; background: none; border: none; cursor: pointer; font-size: 13px;">
...
// =========================================================================
// MUNICIPAL ACCOUNTING OFFICE & GSO COA RPCPPE REPORT GENERATOR (ADMIN)
// =========================================================================

/**
 * Toggles an inventory property item's payment voucher tally status.
 */
window.toggleItemTallyStatus = async function(itemId) {
    ...
    try {
        await database.ref(`inventoryData/${itemId}`).update({
            ...
            talliedBy: (newStatus === 'Tallied') ? 'Admin / Accounting' : null,
        ...
        item.talliedBy = (newStatus === 'Tallied') ? 'Admin / Accounting' : null;
        ...
        window.showGTrackToast(
            ...
            `Property item "${item.propertyNo || item.article}" ${newStatus === 'Tallied' ? 'marked as verified & tallied against the payment voucher.' : 'marked as pending voucher verification.'}`
        );
    } catch (err) {
...
window.exportCOARPCPPEReport = function() {
    ...
    lines.push(`"Prepared & Reconciled By:","Municipal Accounting Office / Property Officer","Municipal Accounting Office","Date: ${reportDate}"`);
